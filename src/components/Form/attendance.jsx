        ...
        if (!_id) {
          ...
          if (assignData && assignData.assignment) {
            ...
            setFormData(prev => ({
              ...
              status: 'Present',
              special_day_type: 'None',
              planned_start_time: isSplit ? slots[0].start_time : start,
              planned_end_time: isSplit ? slots[0].end_time : end,
              is_overnight: isSplit ? slots[0].is_overnight : isOvernight,
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #ffffff 0%, #3498db 100%)', padding: '20px', position: 'relative' }}>
      ...
      <div style={{ maxWidth: '1200px', margin: '80px auto 20px', backgroundColor: '#ffffff', padding: '30px', borderRadius: '15px', boxShadow: '0 4px 12px rgba(0, 0, 0, 0.1)' }}>
        ...
        {message && <div style={{ background: 'linear-gradient(135deg, #d4edda 0%, #c8e6c9 100%)', color: '#155724', padding: '15px', borderRadius: '10px', marginBottom: '20px', textAlign: 'center', border: '1px solid #28a745', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px' }}><FaCheck /> {message}</div>}
